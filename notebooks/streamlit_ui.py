...
MODEL_PATH = os.path.join(ROOT_DIR, "ml_model", "decision_tree_model.pkl")

# Check if model file exists, if not, handle gracefully
if not os.path.exists(MODEL_PATH):
    st.error(f"Model file not found at: {MODEL_PATH}")
    st.info("Please run the decision_tree_model.ipynb notebook first to generate the model.")
    
    # Create directories if they don't exist
    os.makedirs(os.path.join(ROOT_DIR, "ml_model"), exist_ok=True)
...
try:
    ...
    st.info("Please check if the model was properly trained and saved.")
    st.stop()
